...
def dcom(zem, f, cosmo_params):
    #dcomresult = scipy.integrate.quad(dcom_int,0,zem,args=(f, fstar, 
    # Bcoeffs_vec, nvals, cosmo_params, Bfunc, GRflag))[0]
...
def scalar_product_log(hf, gf, psd, freqs):
    logf    = np.log10(freqs)
    dlogf   = logf[1]-logf[0]
    summ    = 0.8*2.*np.log(10)* integrate.simps(np.real(
                (hf*np.conjugate(gf)+np.conjugate(hf)*gf)/psd*freqs))*dlogf
    return summ
...
def phase_PN(farr, Mz, eta, zem, t_c=0, Psi_c=0, free_Psictc=True):
    ...
    if free_Psictc == False:
        t_c     = - 5*Mz/96 * uc**(-8./3) * ( -3./8 + 1./2*Q_1*uc**(2./3) 
                    + 3./5*Q_1p5*uc - 3./4*(Q_1**2 - Q_2)*uc**(4./3) 
                    - (2*Q_1*Q_1p5 - Q_2p5)*uc**(5./3) )

        Psi_c   = 5./48 *uc**(-5./3) *( 9./40 - 1./2*Q_1*uc**(2./3) 
                    - 9./10*Q_1p5*uc + 9./4*(Q_1**2 - Q_2) *uc**(4./3) 
                    - (2*Q_1*Q_1p5 - Q_2p5)*uc**(5./3)*np.log(uc) 
                    ) + 2*np.pi*fc*t_c

    Psi = 5./48 *uarr**(-5./3) *( 9./40 - 1./2*Q_1*uarr**(2./3) 
            - 9./10*Q_1p5*uarr + 9./4*(Q_1**2 - Q_2) *uarr**(4./3) 
            - (2*Q_1*Q_1p5 - Q_2p5)*uarr**(5./3)*np.log(uarr) 
            ) + 2*np.pi*farr*t_c - Psi_c - np.pi/4
...
def Psi_Delta_exact(fo, fstar, c0, Mz, eta, zem, cosmo, tc, psic, 
        cT_type='EFT', width=0):
    ...
    if cT_type=='EFT':
        ...
        d_Delta_dfo_func = lambda fo_func: Delta_step(fo_func, c0, zem, 
                            fstar, width)
    else:
        Delta = 0.
    ...
    to_int  = 5*np.pi*(Mz)**2/96 / (1-Delta)**2 * (1 
                + fo/(1-Delta)*d_Delta_dfo) * uo_arr**(-11./3) * (
    ...
    for i in range(len(fo)):
        ...
        Psi_old[i]  = integrate.simps(-Psi_int[i:], x=fo[i:]
                        )- np.pi/4 + 2*np.pi*fo[i]*tc - psic
        Psi[i]      = integrate.simps(-Psi_int[i:], x=fo[i:]
                        ) - np.pi/4 + 2*np.pi*fo[i]*(tc 
                            + Mpc *dist_val/c*(1-c0)*(1-fstar/fo[i]) 
                            *np.heaviside(fstar-fo[i],1)) - psic


    return [Psi, Psi_old]

# amplitude for exact Delta
def amp_Delta_exact(fo, fstar, c0, Mz, eta, zem, cosmo_params, 
        cT_type='EFT', width=0):
...
def ellipse_para(sigma_x_sq, sigma_y_sq, sigma_xy):
    ...
    a_sq    = (sigma_x_sq+sigma_y_sq)/2 + np.sqrt(
                (sigma_x_sq-sigma_y_sq)**2/4 + sigma_xy**2)
    b_sq    = (sigma_x_sq+sigma_y_sq)/2 - np.sqrt(
                (sigma_x_sq-sigma_y_sq)**2/4 + sigma_xy**2)
